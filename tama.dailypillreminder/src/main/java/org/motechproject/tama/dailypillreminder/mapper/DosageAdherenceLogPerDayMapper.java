...
public class DosageAdherenceLogPerDayMapper {
    ...
    public List<DosageAdherenceLogPerDay> map(String patientDocId, List<DosageAdherenceLogSummary> dosageAdherenceLogSummaries) {
        ...
        for (LocalDate date : map.keySet()) {
            list.add(new DosageAdherenceLogPerDay().setPatientDocId(patientDocId).setDate(date).setLogs(map.get(date)));
        }
    ...
    private HashMap<LocalDate, List<DosageAdherenceLogSummary>> constructLogsPerDayMap(List<DosageAdherenceLogSummary> dosageAdherenceLogSummaries) {
        ...
        for (DosageAdherenceLogSummary summary : dosageAdherenceLogSummaries) {
            if (map.get(summary.getDosageDate()) == null)
                map.put(summary.getDosageDate(), new ArrayList<DosageAdherenceLogSummary>());
