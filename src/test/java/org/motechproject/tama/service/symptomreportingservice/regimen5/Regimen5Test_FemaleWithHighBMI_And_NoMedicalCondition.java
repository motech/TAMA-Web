...
public class Regimen5Test_FemaleWithHighBMI_And_NoMedicalCondition extends SpringIntegrationTest {
    ...
    @Before
    public void setUp() {
        medicalConditionsCase1 = MedicalConditionBuilder.startRecording().ForRegimen5().FemaleWithHighBMI().NoHistoryOfMedicalConditions().AdviceIsNotWithin6And12Months().build();
        medicalConditionsCase2 = MedicalConditionBuilder.startRecording().ForRegimen5().FemaleWithHighBMI().HistoryOfTuberculosis().AdviceIsNotWithin6And12Months().build();
