...
var SchedulePopup = function(id, titleText) {
    ...
    var futureDates = function(d) {
        var startDate = new Date();
        startDate.setDate(startDate.getDate() - 1);
        return startDate.getTime() > d.getTime();
    }

    var allDates = function(d) {
        return false;
    }
    ...
    this.show = function(element, showOptions /*bool*/, okHandler) {
        ...
        var disablePastDates = showOptions && showOptions['disablePastDates'];

        var isDisabledDate = null;
        if (disablePastDates === false) {
            calendar.isDisabledDate = allDates;
        } else {
            calendar.isDisabledDate = futureDates;
        }
