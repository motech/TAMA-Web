...
public class AllCallLogs extends AbstractCouchRepository<CallLog> {
    ...
    public List<CallLog> findCallLogsForDateRangeAndClinic(CallLogSearch callLogSearch) {
        ComplexKey startDosageDateKey = ComplexKey.of(callLogSearch.getCallLogType().name(), callLogSearch.getClinicId(), callLogSearch.getFromDate());
        ComplexKey endDosageDateKey = ComplexKey.of(callLogSearch.getCallLogType().name(), callLogSearch.getClinicId(), callLogSearch.getToDate());
    ...
    public List<CallLog> findCallLogsForDateRange(CallLogSearch callLogSearch) {
        ComplexKey startDosageDateKey = ComplexKey.of(callLogSearch.getCallLogType().name(), callLogSearch.getFromDate());
        ComplexKey endDosageDateKey = ComplexKey.of(callLogSearch.getCallLogType().name(), callLogSearch.getToDate());
    ...
    @View(name = "find_all_call_logs_between_a_given_date_range", map = "function(doc) { if(doc.documentType == 'CallLog') { var callType = 'Answered'; if(doc.callEvents) { for(var idx in doc.callEvents) { if (doc.callEvents[idx].name == 'Missed') { callType = 'Missed'; } } } emit([callType, doc.startTime], doc._id); } }", reduce = "_count")
    ...
    @View(name = "find_all_call_logs_between_a_given_date_range_and_by_clinicId", map = "function(doc) { if(doc.documentType == 'CallLog') { var callType = 'Answered'; if(doc.callEvents) { for(var idx in doc.callEvents) { if (doc.callEvents[idx].name == 'Missed') { callType = 'Missed'; } } } emit([callType, doc.clinicId, doc.startTime], doc._id); } }", reduce = "_count")
