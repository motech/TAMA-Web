...
public class EditPatientIdInDataBase {
	...
	//private static final Logger LOG = LoggerFactory.getLogger(Schemes.class);
	 ...
	 public void migrate(String filePath) {
	        ...
	        try{
	            ...
	            for (String row : fileContents) {
	                String[] rowContents = StringUtils.tokenizeToStringArray(row, DELIMITER,false,false);
	                ...
	                if(hasPatientId == false)
	                	System.out.printf("Patient Id %s is not in the couch Db .",oldPatientId);
	            }
	            editPatientIdInPsqlRevised(fileContents);
	        }
	...
	public void editPatientsIdInCallLogDoc(String oldPatientId,String clinicId,String newPatientId){    
		callLogSearch = new CallLogSearch(fromDate, toDate, CallLog.CallLogType.Answered, oldPatientId.toLowerCase(), true, clinicId);
			allCallLogsNew.editPatientId(callLogSearch, newPatientId);
			callLogSearch = new CallLogSearch(fromDate, toDate, CallLog.CallLogType.Missed, oldPatientId.toLowerCase(), true, clinicId);
			allCallLogsNew.editPatientId(callLogSearch, newPatientId);
	}
	
	
	public void editPatientIdInPsqlRevised(List<String> fileContents){
	       Connection c = null;
	       Statement stmt = null;
